...
if __name__ == '__main__':
    ...
    for _ in range(3):
        for __ in range(MAX_ENTITY_SPAWN):
            if _ == 0:
                rock = Entity(EntityType.ROCK, ROCK_IMG)
                entities_sprites.add(rock)
            elif _ == 1:
                paper = Entity(EntityType.PAPER, PAPER_IMG)
                entities_sprites.add(paper)
            elif _ == 2:
                scissors = Entity( EntityType.SCISSORS, SCISSORS_IMG)
    ...
    while True:
        ...
        for entity in entities_sprites:
            ...
            if entity.type == EntityType.ROCK.value: #type: ignore
                entity.update(scissors_list, rock_list) #type: ignore
            elif entity.type == EntityType.PAPER.value: #type: ignore
                entity.update(rock_list, paper_list)
            elif entity.type == EntityType.SCISSORS.value: #type: ignore
                entity.update(paper_list, scissors_list)
